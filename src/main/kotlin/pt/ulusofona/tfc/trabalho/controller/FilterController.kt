...
class FilterController(val jdbcTemplate: JdbcTemplate,
    ...
    fun filter(@RequestParam("fields") fields: List<String>?,
        ...
        if (fields == null){
            bindingResult.rejectValue("fields", "inexistentValue", "Erro: O campo têm de ser preenchido")
            return "filter"
        }

        if (values == null){
            bindingResult.rejectValue("values", "inexistentValue", "Erro: O campo têm de ser preenchido")
            return "filter"
        }

        if (fields[0] != "s"){
            bindingResult.rejectValue("values", "incorretValue", "Erro: O campo está preenchido incorretamente")
            return "filter"
        }
        ...
        for(field in fields){
            ...
            valor = if (values[i] == "SIM") true else false
